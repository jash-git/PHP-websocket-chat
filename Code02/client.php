...
<html>
	...
	<body>
		...
		<script language="javascript" type="text/javascript">  
			//create a new WebSocket object. [創建一個新的 WebSocket 對象。]
			var msgBox = $('#message-box');
			var wsUri = "ws://localhost:9000/PHP_websocket/server.php"; 	
			websocket = new WebSocket(wsUri); 
			
			websocket.onopen = function(ev) { // connection is open [連接連線成功顯示訊息]
				msgBox.append('<div class="system_msg" style="color:#bbbbbb">Welcome to my "Demo WebSocket Chat box"!</div>'); //notify user
			}
			// Message received from server [從服務器收到的消息]
			websocket.onmessage = function(ev)
			{
				var response 		= JSON.parse(ev.data); //PHP sends Json data [解JSON]
